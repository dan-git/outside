...
#define START_FRAME 100 // min value is 1 since we to use the 0th frame for setting up
...
int main(int argc, char* argv[])
{
   ...
   if (camera_.isOpened())  printf("video opened \n");
   else
   {
   ...
   */
   ...
   // read a frame
   Mat nextFrame;
   //  capture some frames for outside analysis
   for (int i=0; i<60; i++)
   {
      capture(&nextFrame);
      waitKey(30);  // give highgui time to get the image
   ...
   for (int i=0; i<50; i++)
   {
      ...
      if (!(i % 5))
      {
         buffer << "/home/dbarry/Dropbox/outdoor_bot/media/image_processing/ceiling_image" << i << ".jpg";
