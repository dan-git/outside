...
void publishState(void)
{
	...
	if (pub_tilt_status.getNumSubscribers() > 0)
	{
        ...
        // Status codes: 0 = not moving, 1 = endstop at max or min angle,
        // 4 = moving, 8 = obstacle, momentary pause, -1 = unknown state 
	}
}

// test with 
// rostopic pub /tilt_angle std_msgs/Float64 -- 0
// replacing 0 with a value in the range [-31:31] to get that angle of tilt
// Note that the kinect uses its accelerometer to produce a tilt angle with
// respect to the horizon.  If you mount the kinect not level, you will have to
// bias the MAX and MIN tilt angles, or get rid of the safety check statement:
...
// test with 
// rostopic pub /led_option std_msgs/UInt16 -- 0
// replacing 0 with a value in the range [0:7] where
// 0 = off, 1 = green, 2 = red, 3 = orange, 4 or 5 = blinking green, 6 or 7 = red/orange blink
...
int main(int argc, char* argv[])
{
	...
	pub_imu = n.advertise<sensor_msgs::Imu>("imu_kinect", 15); // original code published on "imu" topic
