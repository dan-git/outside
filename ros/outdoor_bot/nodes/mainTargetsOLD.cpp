...
private:
   ...
   ros::Publisher target_center_pub_;
	//void FindmainTargets(int numVertices, double MinAngle, double MaxAngle, int MinArea, int MaxArea);
   ...
   :  nh_(nh)
{
      ...
      range_ = 0.;
      //filename_ = "/target/dbarry/Dropbox/outdoor_bot/media/image_processing/digcam0";
      //filename_ = "/target/dbarry/Dropbox/outdoor_bot/media/image_processing/hexagon_cropped";
      //std::string filename = "/target/dbarry/Dropbox/outdoor_bot/media/image_processing/digcam0.jpg";
      //std::string filename = "/target/dbarry/Dropbox/outdoor_bot/media/image_processing/hexagon_cropped.jpg";
      //Mat image = imread(filename, CV_LOAD_IMAGE_UNCHANGED);
      //waitKey(30);
      // GetmainTargets parameters are:
      ...
      //GetmainTargets(image, 6, 0.05, 0.42, 0.28, 0.63, 0.5, minArea, maxArea,
      //   minRatioAreaArcLength, maxRatioAreaArcLength, 0.55, 0.72, false, false);
      //GetmainTargets(image, 6, 0.05, 0.42, 0.28, 0.63, 0.50, 5000., 3000000.,
      //     10., 120., 0.55, 0.75, false, false);

      //GetmainTargets(image, 6, 0.2, 0.45, 0.28, 0.7, 0.49, 100., 2000000.,
...
CvRect mainTargets::GetTargetRectangle()
{
...
bool GetmainTargets(Mat NavImage, int numVertices, double MaxMatchValue,
...
{
   if (numVertices < 1) {
      ROS_ERROR("Number of Vertices requested is less than 1, exiting mainTargets \n");
      return false;
   }
   ...
   // model target target, aspect ratio = .667, width = 200, center = (320,240)
   ...
   */ 
   ...
   if (numTargetsFound < 1)
   {
      ...
      outdoor_bot::mainTargets_msg fail_message;
      fail_message.centerX = -1;
      fail_message.centerY = -1;
      fail_message.range = -1;
      target_center_pub_.publish(fail_message);
      return false;
   }
   ...
   outdoor_bot::mainTargets_msg target_center_message;
   target_center_message.centerX = centerX_;
   target_center_message.centerY = centerY_;
   target_center_message.totalX = NavImage.cols;
   target_center_message.range = range_;
   target_center_pub_.publish(target_center_message);
   cout << "target center published = (" << centerX_ << ", " << centerY_ << ")" << endl;
   cout << "target range published = " << range_ << " m"  << endl;
   ...
   std::string filename_contour = "/target/dbarry/Dropbox/outdoor_bot/media/image_processing/contour.jpg";
...
bool sendCenter(outdoor_bot::mainTargets_service::Request  &req, outdoor_bot::mainTargets_service::Response &resp)
{
   ...
   if (GetmainTargets(image, 6, 0.05, 0.42, 0.28, 0.63, 0.50, 5000., 3000000.,10., 120., 0.55, 0.75, false, false))
   {
   ...
   // got an image of the target target.  analyze it and publish the center point
   void targetTargetCallback(const sensor_msgs::ImageConstPtr& msg)
   {
      //ROS_INFO("image received");
      Mat target_image;
      try
      {
       target_image = cv_bridge::toCvShare(msg, "bgr8")->image;
      ...
      if (GetmainTargets(target_image, 6, 0.05, 0.42, 0.28, 0.63, 0.50, 5000., 3000000.,10., 120., 0.55, 0.75, false, false))
      {
         ROS_INFO("Found target target: (x,y), range = (%d, %d), %f", centerX_, centerY_, range_);
      }
      else ROS_INFO("Center not found in target target image");
   }
};

/*
int mainTargets::RecognizeTarget(IplImage *pMyImage)
{
	//cvNamedWindow("ROI",1);
	wxString TempString;
	bool PolygonPresent;
...
/*
	...
	//int AreaAvgWholeTarget = areaAvg_;  // use this because GetmainTargets will change areaAvg_ to correspond
								// to the target square rather than the area inside the whole target
	//PolygonPresent = GetmainTargets(pMyImageROI,4,0,1,(int) (AreaAvgWholeTarget/100),(int) (AreaAvgWholeTarget/20),
...
/*
		...
		//PolygonPresent = GetmainTargets(pImageROIQuadrant,4,0,0.2,(int) (AreaAvgWholeTarget/100),
...
bool mainTargets::FindTemplate(IplImage * TargetImage, IplImage * TemplateImage, float Threshold)
{
	//wxString TempString;
...
bool mainTargets::FindDarkSquare(IplImage * MyImage, int SquareSizeFactor, float Threshold)
{
	//wxString TempString;
...
IplImage * mainTargets::CreateTemplate(IplImage *InternalTarget)
{
...
/*
bool mainTargets::GetCircleTarget(IplImage *img, int *pCircleCenterX, int *pCircleCenterY, int *pCircleRadius)
{
		wxString TempString;
		float* p;
...
int main(int argc, char* argv[])
{
   ...
   mainTargets nt(nh);
   ros::ServiceServer service = nh.advertiseService("mainTargetsService", &mainTargets::sendCenter, &nt);
   image_transport::ImageTransport it(nh);
   image_transport::Subscriber target_image_sub =
         it.subscribe("target_target_image", 1, &mainTargets::targetTargetCallback, &nt);
   //cv::namedWindow("view");
